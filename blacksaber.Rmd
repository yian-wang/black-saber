...
hiring1_gender_model = glmer(next_round ~ cv + gpa + cover_letter + (1 | gender),#(1 + genre | artist_name), 
...
hiring1_team_gender = glmer(next_round ~ cv + gpa + cover_letter + (1 + team_applied_for | gender),
                data = round1_2, family = binomial(link = "logit"), nAGQ=0)
...
<!-- below are additional models and tests but we see that adding these additional random effects are not significant -->
```{r}
hiring1_team_model = glmer(next_round ~ cv + gpa + cover_letter + (1 | team_applied_for),#(1 + genre | artist_name), 
                data = round1_2, family = binomial(link = "logit"), nAGQ=0)
summary(hiring1_team_model)

hiring1_teamgender_model = glmer(next_round ~ cv + gpa + cover_letter + (1 | team_applied_for) + (1 | gender),#(1 + genre | artist_name), 
                data = round1_2, family = binomial(link = "logit"), nAGQ=0)
summary(hiring1_teamgender_model)

lmtest::lrtest(hiring1_team_model, hiring1_teamgender_model)
```
...
# add fixed effect for gender
hiring1_gender_model = glmer(next_round ~ cv + gpa + cover_letter + gender + (1 | team_applied_for),#(1 + genre | artist_name), 
                data = round1_2, family = binomial(link = "logit"), nAGQ=0)
summary(hiring1_gender_model)

#let's see if we add an effect for team applied to - maybe the process for data and software are different?
hiring1_team_gender = glmer(next_round ~ cv + gpa + cover_letter + (1 + gender | team_applied_for),
                data = round1_2, family = binomial(link = "logit"), nAGQ=0)
...
<!-- okay idk i need to figure out fixed vs random effects tmrw - which one should gender be? bc i thought it was like the (1| gender) one but thats random effect but im pretty sure thats a fixed effect...... fuckkkkkkkkkk meeeee  -->
<!-- i guess yeah what i have above is promising? -->
<!-- where its (1|team applied for) vs (1+gender|team) so im comparing differences in teams bc there might be differences there but seeing if gender affects these decisions? but like what happens if theres no difference b/w simple model and random effect for team? cam i add a fixed effect without a random effect?  -->
...
#lets visualize the indep variables real quick
plot <- round2_3 %>% 
  ggplot(aes(x = technical_skills)) +
  geom_histogram(colour = "grey", fill = "#6C3082") +
  theme_minimal() +
...
plot4 <- round2_3 %>% 
  ggplot(aes(x = leadership_presence)) +
  geom_histogram(colour = "grey", fill = "#6C3082") +
  theme_minimal() +
  labs(title = "Look! A title. But do you want a title or a caption for your report?", x = "This is a changed x label")
plot4
...
# add fixed effect for gender
hiring2_gender_model = glmer(next_round ~ technical_skills + writing_skills + leadership_presence + speaking_skills + (1 | gender), data = round2_3, family = binomial(link = "logit"), nAGQ=0)

summary(hiring2_gender_model)

#let's see if we add an effect for team applied to - maybe the process for data and software are different?
hiring2_team_gender = glmer(next_round ~ technical_skills + writing_skills + leadership_presence + speaking_skills + (1 + team_applied_for | gender),
...
```{r}
model = glmer(hired ~ avg_interview + gender + (1|team_applied_for), family = binomial(link='logit'), data = round3_hired, nAGQ =0)
summary(model)
...
```{r}
fixef(model) # males have lower pitches than females by an avg of 109hzsdfskdgls
...
we probably see a discrepency because of the systematic barriers women face in other professional spheres - the algorithm doesnt rate biasedly, though
