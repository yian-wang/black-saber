...
library(lme4)
promotions <- promotions %>% mutate(gender = factor(gender)) %>% mutate_if(is.factor, as.numeric) %>% group_by(employee_id, role_seniority)
#make role seniority into numbers if id are the same and the next role seniority > than previous add 1 to promo count 
glimpse(promotions)
simple <- lm(role_seniority ~ gender, data = promotions)

promo <- lmer(role_seniority ~ (1|employee_id), data = promotions)
...
salary <- lmer(salary ~ role_seniority + leadership_for_level + productivity + gender +(1|financial_q), data = promotions) #employee IDs are not independent since this data set is over years so random effect on employee id and financial quarter? 
```
